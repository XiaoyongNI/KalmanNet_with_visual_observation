...
from visual_supplementary import y_size, decoaded_dimention, H_visual_function_for_creating_data
...
class SystemModel:
    ...
    def GenerateSequence(self, Q_gen, R_gen, T):
        ...
        for t in range(0, T):
            ...
            # Observation Noise
            if self.r == 0:
                yt = H_visual_function_for_creating_data(xt)
            else:
                yt = H_visual_function_for_creating_data(xt)
                #yt_nump = yt.cpu().detach().numpy()
                #plt.imshow(yt_nump)

                yt_vec = torch.reshape(yt, (yt.shape[0]*yt.shape[0],1))
    ...
    def GenerateBatch(self, size, T, randomInit=False, seqInit=False, T_test=0):
        ...
        for i in range(0, size):
            ...
            print("generated seq number {}".format(i))
